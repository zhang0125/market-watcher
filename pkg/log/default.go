...
// logFile is the log file object
var logErrorFile io.WriteCloser
...
func init() {
	...
	Log, _ = NewFileLogger("text", LogLevelInfo)
	ErrLog, _ = NewFileLogger("text", LogLevelError)
}
...
func NewFileLogger(format, level string) (Logger, error) {
	...
	case level == LogLevelInfo:
		zl = zerolog.New(logFile).Level(logLevel).With().Timestamp().Logger()
	default:
		zl = zerolog.New(logErrorFile).Level(logLevel).With().Timestamp().Logger()
	}
